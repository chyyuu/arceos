...
impl MountedFsList {
    ...
    pub fn umount(&mut self, mounted_fs: &MountedFileSystem) {
        self.0.retain(|fs| 
            fs.path == mounted_fs.path && fs.fs.name() == mounted_fs.fs.name()
        );
    }
    ...
    pub fn mount(&mut self, path: &str, source_fs: Arc<dyn VfsFileSystem>) {
        ...
        let search = self.0.iter().find(
            |fs| fs.path == path && fs.fs.name() == source_fs.name()
        );
        
        // mount if it not been mounted.
        if search.is_none() {
            self.0.push(MountedFileSystem::new(path.to_string(), source_fs.clone()))
        }
    }

    /// get a matched fiel system by path
    pub fn get_matched_fs(&self, path: &str) -> Option<&MountedFileSystem> {
        let (_, index) = self.0.iter().enumerate().fold((0, usize::MAX), |(len, index), (i, x)| {
